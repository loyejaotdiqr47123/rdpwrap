...
DWORD INIReadDWordHex(INI_FILE* IniFile, char* Sect, char* VariableName, PLATFORM_DWORD Default)
{
	...
	{
		return static_cast<DWORD>(Variable.ValueHex);
	}
...
void WriteToLog(LPSTR Text)
{
	...
	WriteFile(hFile, Text, static_cast<DWORD>(strlen(Text)), &dwBytesOfWritten, NULL);
	CloseHandle(hFile);
}
...
bool OverrideSL(LPWSTR ValueName, DWORD* Value)
{
	...
	if (IniFile->VariableExists(L"SLPolicy", ValueName))
	{
		...
		else *Value = static_cast<DWORD>(Variable.ValueDec);
		return true;
	}
	return false;
}
...
void Hook()
{
	...
	logStream << "Base addr:  0x" << std::hex << hTermSrv << "\r\n"
		<< "SvcMain:    termsrv.dll+0x" << std::hex << (PLATFORM_DWORD)_ServiceMain - (PLATFORM_DWORD)hTermSrv << "\r\n"
		<< "SvcGlobals: termsrv.dll+0x" << std::hex << (PLATFORM_DWORD)_SvchostPushServiceGlobals - (PLATFORM_DWORD)hTermSrv << "\r\n";
	WriteToLog(const_cast<LPSTR>(logStream.str().c_str()));
